...
t0      = 0;
tf_G    = 3.5;             % when time guess bigger than 6.4 the result will collapse
lamG    = [-1 0 0 1 0];    % initial guess
...
lam = fsolve(@Error_Orbital,lam_G,option,r0,rf,theta0,vr0,vrf,vtheta0,vthetaf,m0,t0,u,T,ve);
toc
disp(['runtime: ',num2str(toc)]); % display computation time

[E,t,p] = Error_Orbital(lam,r0,rf,theta0,vr0,vrf,vtheta0,vthetaf,m0,t0,u,T,ve)
% plot all states
figure(1);
plot(t,p(:,1),'r*-');
hold on;
...
%% Error Function
function [E,t,P] = Error_Orbital(lam_G,r0,rf,theta0,vr0,vrf,vtheta0,vthetaf,m0,t0,u,T,ve)

lam_r0_G      = lam_G(1); 
lam_theta0_G  = lam_G(2);
lam_vr0_G     = lam_G(3); 
lam_vtheta0_G = lam_G(4); 
lam_m0_G      = lam_G(5); 
tf_G          = lam_G(6);
P0 = [r0; theta0; vr0; vtheta0; m0; lam_r0_G; lam_theta0_G; lam_vr0_G; lam_vtheta0_G; lam_m0_G];

option    = odeset('RelTol',1e-8);
[t,P]     = ode113(@myOde_Orbital,[t0 tf_G],P0,option,u,T,ve);
Ptf       = P(end,:);
r_tf      = Ptf(1);
theta_tf  = Ptf(2);
vr_tf     = Ptf(3);
...
beta    = atan2(lam_vr_tf,lam_vtheta_tf); % control
beta    = unwrap(beta);                   % angle will be unwrapped
...
E   = [r_tf-rf; vr_tf-vrf; vtheta_tf-vthetaf; lam_theta_tf; lam_m_tf-1; Htf];

end

%% State and Costate Function
function P_dot = myOde_Orbital(t,P,u,T,ve)
...
P_dot = [r_dot; theta_dot; vr_dot; vtheta_dot; m_dot;...
